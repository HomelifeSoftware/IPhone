...
function renderProducts(productsToRender) {
  ...
  if (productsToRender.length === 0) {
    productsGrid.innerHTML =
      '<p style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-light);">No products found matching your criteria.</p>';
    return;
  }
      ...
      product.name
            ...
            <div class="product-info">
                ...
                <div class="product-price-container">
                    ${
                      product.discount > 0
                        ? `<span class="original-price">TSh ${product.price.toLocaleString()}</span>`
                        : ""
                    }
                    <span class="final-price">TSh ${finalPrice.toLocaleString()}</span>
                </div>
                <button class="read-more-btn" onclick="showProductDetail(${
                  product.id
                })">
                    <span>Read More</span>
                </button>
...
async function addToCart(productId) {
  ...
  const existingItem = cart.find(
    (item) => item.id == productId || String(item.id) === String(productId)
  );
  if (existingItem) {
      ...
      (item) => `
        <div class="cart-item" data-item-id="${item.id}">
            ...
            <div class="cart-item-info">
                ...
                <div class="cart-item-subtotal">Subtotal: TSh ${(
                  item.finalPrice * item.quantity
                ).toLocaleString()}</div>
                <div class="quantity-controls">
                    <button class="quantity-btn quantity-decrease" onclick="updateQuantity(${
                      item.id
                    }, -1)" title="Decrease quantity">
                        <span class="material-icons">remove</span>
                    </button>
                    <span class="quantity-value">${item.quantity}</span>
                    <button class="quantity-btn quantity-increase" onclick="updateQuantity(${
                      item.id
                    }, 1)" title="Increase quantity">
                        <span class="material-icons">add</span>
                    </button>
                </div>
                <button class="remove-item-btn" onclick="confirmRemoveFromCart(${
                  item.id
                }, '${item.name.replace(/'/g, "\\'")}')" title="Remove item">
